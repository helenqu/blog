...
\title{RL's Deadly Triad Meets Optimization}
\author{Helen Qu}
...
This unsavory combination of \textbf{function approximation, off-policy learning, and bootstrapping} leads to unstable learning dynamics, even in the simplest of systems.
...
\section{Why adopt Deadly Triad learning paradigms?}

If the deadly triad is so deadly, why not avoid it altogether? 
\begin{itemize}
    \item \textbf{Function approximation} (e.g., via neural networks) allows us to parameterize very high-dimensional state-action spaces that would otherwise fall prey to the curse of dimensionality (e.g., DQN\footnote{\url{https://arxiv.org/abs/1509.06461}}). Of course, real-world state-action spaces (e.g., multimodal video + sensor inputs in self-driving) are fundamentally high-dimensional so this problem isn't going away.
    \item \textbf{Off-policy learning} is a convenient way to disentangle the data source (behavior policy) from the target policy. Since the optimal policy for data generation will rarely coincide with the globally optimal policy due to the need for exploration, this allows the data generation policy to decouple from the learned optimal target policy. Additionally, true on-policy learning is difficult to implement in practice at scale in distributed setups.
    \item Finally, \textbf{bootstrapping} replaces the true return at state $s_t$, which requires rolling out a full trajectory to calculate, with a one-step rollout ($r_t$) and approximates the rest with the bootstrapped value function estimate $\V(\snext)$. This is a straightforward way to balance the trade-off between computational cost and accuracy.
\end{itemize}

\section{Setup}

We adopt the learning setup of \textbf{Baird's counterexample}\footnote{\url{http://leemon.com/papers/1995b2.pdf}}, an embarrassingly simple concrete system (with no rewards!) that demonstrates the instability of semi-gradient Q-learning in the presence of the deadly triad. I'll introduce the relevant ingredients here.
...
\begin{itemize}
    \item \textbf{Function approximation}: We use a linear parameterization for the value function $\V$.
    \item \textbf{Bootstrapping}: The TD family of methods uses bootstrapping to iteratively refine value function estimates (as opposed to, e.g., Monte Carlo estimation).
    \item \textbf{Off-policy learning}: Q-learning is an off-policy variant of TD learning.
\end{itemize}

\section{The TD Algorithm}
...
\section{Insights from Optimization Theory}
...
$$
which evolves as $e_{t+1} = \itmtx e_t \Rightarrow e_t = \itmtx^t e_0$.\footnote{Quick derivation of $e_{t+1} = (\Ibf - \eta \Abf)e_t$: 
$\begin{aligned}
e_{t+1} &= \theta_{t+1} - \tstar \\
&= \theta_t + \eta (\cbf - \Abf \theta_t) - \tstar \\
&= \eta \Abf \tstar + (\Ibf - \eta \Abf) \theta_t - \tstar \\
&= (\Ibf - \eta \Abf)(\theta_t - \tstar) \\
&= (\Ibf - \eta \Abf) e_t
\end{aligned}$} We can define convergence as $\lim_{t \to \infty} \norm{e_t} = 0$, meaning that we require 
$$
    \lim_{t \to \infty} \itmtx^t = 0 \Rightarrow \rho(\itmtx) < 1
$$
to guarantee convergence.\footnote{This implication was not directly obvious for me, so I'll write out a quick proof. We know $\rho(\Mbf) = \max_{\lambda} |\lambda|$, and for any eigenvalue $\lambda$ of matrix $\Mbf$ with associated eigenvector $\mathbf{v}$, $\Mbf \mathbf{v} = \lambda \mathbf{v}$ and $\Mbf^t \mathbf{v} = \lambda^t \mathbf{v}$. Thus, $\Mbf^t \mathbf{v} \rightarrow 0 \Rightarrow \lambda^t \rightarrow 0$. This directly implies $\max(\lambda) < 1 \Rightarrow \rho(\Mbf) < 1$.}
...
We can prove that $\AMC$ is SPD. We define $\xbf \in \text{span}(\AMC) \neq 0$ and show that $\transpose{\xbf} \AMC \xbf > 0$:
$$
    \begin{aligned}
  \transpose{\xbf} \AMC \xbf &= \transpose{\xbf} \transpose{\Phi} D_b \Phi \xbf \cr
...
We can write $\norm[D_{\pi}]{P_{\pi}\ubf}$ as an expected value and apply Jensen's inequality since $x^2$ is a convex function of $x$:
$$
    \begin{aligned}
    ...
    \end{aligned}    
$$
Since $d_{\pi}$ and $P_{\pi}$ are defined for the same policy $\pi$, $d_{\pi}$ is \emph{stationary} for $P_{\pi}$: $\transpose{\dpi} \Ppi = \transpose{\dpi}$. This gives us
$$
...
$$
Now we see that the expected TD(0) update can be written simply in terms of $\Tpi$\footnote{We note that when target policy $\pi$ and behavior policy $b$ have very different distributions over actions/states, importance sampling with weighting $\frac{\pi(a_t \mid s_t)}{b(a_t \mid s_t)}$ must be used to equate $\ev[d_b]{\cdot}$ with $\ev[d_{\pi}]{\cdot}$.}:
$$
